...
def save_pretrained(encoder, decoder, options):
    ...
    save_checkpoint(model, False, options.pre_trained_file)
...
def pretrain(dataloader, encoder, pre_train_decoder, optimizer, vocab_tgt, options):
    ...
    while epoch < options.num_epoch_pretrain:
        ...
        for sample_batched in dataloader:
            ...
            # print('^^^^^^^^^^^^^^^^^^^ CONTROL BATCHES ', control_batches)
            # print(perm_idx)
            ...
            # print('^^^^^^^^^^^^^^^^^^^ CONTROL BATCHES ', control_batches)
            # print('^^^^^^^^^^^^^^^^^^^ CONTROL BATCHES SHAPE ', control_batches.size())
...
def initialize_model(options):
    ...
    if options.resume:
        ...
        if not options.pretrain and load_pretrained:
            print('######## Loading pretrained encoder #######')
            if os.path.isfile(options.pre_trained_encoder_file):
                checkpoint = torch.load(options.pre_trained_encoder_file)
                encoder.load_state_dict(checkpoint['encoder'])
...
def train(options):
    ...
    pretrain_dataset = ControllableUnsupervisedDatasetFromArray(src_lines, None, control_len, "pretrain_data",
                                                                vocab_src,
    ...
    # valid_file_src = options.valid_src
    # valid_inputs = open(valid_file_src, 'r').readlines()
    if options.pretrain:
    ...
    prev_total_reward = 0
    explore_dataloader = pretrain_dataloader
    print('######### Training ######')
    while epoch < options.epochs:
        ...
        for sample_batched in explore_dataloader:
            ...
            try:
                ...
                pass
        # num_batches+=1 #just to smooth
        # av = total_reward / num_batches
        ...
        while ex_epoch < options.num_epoch_pretrain:
            ...
            for sample_batched in dataloaderPre:
                ...
                loss = train_batch(input_batches, input_lengths, output_batches, output_lengths, control_batches,
                                   encoder, decoder,
                                   encoder_optimizer, options, vocab_tgt)
                ex_epoch_loss += loss
        print('Sub Epoch loss ', ex_epoch_loss / batch_num)

        # if (epoch % options.generate_every == 0):
        #    generate(encoder, decoder, valid_inputs, options, vocab_src, vocab_tgt, epoch, False)
...
def main():
    ...
    print(options)
    # options_file = open(options.save_model + '_options.json', 'w')
